...
class AssistantService:
    ...
    def __init__(self, api_key, model_name="gpt-4-turbo-preview"):
        ...
        self.file = self.openai.files.create(
...
Напишите ваш вопрос в чат!" 
        "Твоя основная задача вовлечь клиента в диалог и собрать контактные данные"
                ...
                # "those lines."
                3.Всегда отвечай на русском языке\nНе упоминай об источиках и не делай 【11†источник】 таких пометок"
                ...
                10.Не пиши большой текст оформляй его красиво и периодически добавлять смайлы
...
пример предложения от которого можно отталкиваться :"В нашем международном онлайн-институте более 15 обучающих программ. Мы даем не просто знания, а профессию. Выдаем дипломы, которые позволяют оказывать услуги как в России, так и за рубежом.
...
А также у вас есть шанс получить грант до 60 000 рублей, который покрывает значительную часть обучения.
...
номер:
"Напишите ваш номер телефона и мы закрепим за ним грант на обучение в размере 60 000 рублей
    ...
    def __get_answer_from_messages(self, messages, user_message_id):
        ...
        for message in messages:
            if message.id == user_message_id:
                return messages.data[index-1].content[0].text.value
            index += 1
        return None
